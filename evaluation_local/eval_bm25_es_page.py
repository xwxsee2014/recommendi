...
def load_dataset():
    ...
    # docs now includes metadata_fields
    corpus = {
        doc.doc_id: {
            "text": doc.text,
            "metadata_fields": doc.metadata_fields  # add metadata_fields
        }
...
def setup_es_index(dataset_name, docs, doc_ids):
    ...
    # 创建 mapping，使用 ik_smart 分词器，并添加 metadata_fields
    mapping = {
        "settings": {
            "analysis": {
                "analyzer": {
    ...
    actions = [
        {
            ...
            "_source": {
                "content": docs[i]["text"],
                "doc_id": doc_ids[i],
                "metadata_fields": docs[i]["metadata_fields"]
            }
        }
